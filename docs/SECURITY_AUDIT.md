...
- Public clients use Burner Point API routes instead of calling Twilio, Telnyx, Tremil, Bandwidth, OpenAI, Airalo, Oxylabs, Smartproxy, WireGuard, payment gateways, Resend, Supabase service-role APIs, or private PostHog capture directly.
...
- Supabase Auth is the primary auth provider for OAuth, session security, password recovery, and production identity verification.
...
1. Provider-specific webhook signatures should be fully implemented for Twilio, Telnyx, Bandwidth, and Tremil using each provider's exact signing algorithm and timestamp replay window.
...
7. Database RLS policies exist in Supabase migrations for user-owned tables. Keep backend service-role access isolated and expand direct client access only where policies are verified against `auth.uid()`.
