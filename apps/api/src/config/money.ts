...
export function resolveUsdToNgnRate(source: RuntimeEnvSource): number {
  const configured = Number(readEnv(source, 'FX_USD_TO_NGN_RATE'));
  if (!Number.isFinite(configured) || configured <= 0) {
    throw new Error('A provider-sourced FX rate is required for local conversion');
  }
  return configured;
}
...
export function withWalletPresentation<T extends { walletBalanceKobo?: number; lifetimeSpendKobo?: number }>(
  ...
  const lifetimeSpendKobo = lifetimeSpendUsdCents;
