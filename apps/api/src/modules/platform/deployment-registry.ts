...
export const DEPLOYMENT_TARGETS: DeploymentTargetDefinition[] = [
  ...
  },
  {
    ...
    requiredEnv: ['VERCEL_ENV', 'NEXT_PUBLIC_APP_URL', 'NEXT_PUBLIC_API_URL', 'NEXT_PUBLIC_SUPABASE_URL', 'NEXT_PUBLIC_SUPABASE_ANON_KEY'],
  ...
  },
  {
    ...
    optionalEnv: ['POOLER_URL', 'SUPABASE_URL', 'DB_SSL', 'DB_SSL_REJECT_UNAUTHORIZED'],
    productionRequired: true,
    releaseCheck: 'Migrations are reviewed, reversible where practical, and applied before traffic depends on new schema.',
    rollback: 'Use Supabase backups or a forward-compatible migration fix; avoid destructive rollback on live data.',
  },
  {
    id: 'supabase-auth',
    name: 'Supabase Auth',
    ...
    productionRequired: true,
    releaseCheck: 'Email, phone, Google, Apple, Microsoft, redirect URLs, and Supabase Auth providers are configured per environment.',
    rollback: 'Disable the affected auth route or restore the previous Supabase Auth redirect/provider configuration.',
  },
  {
  ...
  },
  {
    ...
    requiredEnv: ['EXPO_PROJECT_ID', 'EXPO_PUBLIC_API_URL', 'EXPO_PUBLIC_WEB_URL', 'EXPO_PUBLIC_SUPABASE_URL', 'EXPO_PUBLIC_SUPABASE_ANON_KEY'],
  ...
  },
  {
    ...
    stage: 'core',
    environmentScope: ['staging', 'production'],
    ownerSurface: 'Store-managed mobile subscriptions, offerings, customer state, and entitlement sync',
    ...
    productionRequired: true,
    releaseCheck: 'RevenueCat offerings load in iOS and Android builds, purchases and restores sync entitlements, and the webhook reaches /api/webhooks/revenuecat with idempotent processing.',
    rollback: 'Disable the affected RevenueCat offering or entitlement, keep wallet funding live, and ship a forward fix without exposing client secrets.',
  },
  {
    id: 'apple-app-store',
    name: 'iOS App Store',
  ...
  },
  {
    ...
    rollback: 'Remove Telnyx from route selection while keeping Twilio and Bandwidth active.',
  },
  {
    id: 'bandwidth',
    name: 'Bandwidth',
  ...
  },
  {
    ...
    stage: 'primary',
    environmentScope: ['staging', 'production'],
    ownerSurface: 'International card checkout and web subscriptions',
    ...
    productionRequired: true,
    releaseCheck: 'One-time and web subscription webhooks reconcile with ledger state without frontend secret exposure.',
    rollback: 'Pause Paddle products or switch checkout routing while keeping wallet reconciliation intact.',
  },
  {
    id: 'nowpayments',
    name: 'NOWPayments',
  ...
  },
  {
    ...
    stage: 'supporting',
    environmentScope: ['development', 'staging', 'production'],
    ownerSurface: 'Manual Supabase Postgres inspection by trusted operators',
    ...
    rollback: 'Revoke database credentials and rotate Supabase connection strings.',
  },
];
...
export const DEPLOYMENT_ENVIRONMENTS: DeploymentEnvironmentDefinition[] = [
  {
    ...
    serverEnv: ['DATABASE_URL', 'DIRECT_DATABASE_URL', 'REDIS_URL', 'SUPABASE_SERVICE_ROLE_KEY', 'TWILIO_* sandbox/test values'],
  ...
  },
  {
    ...
    secretStores: ['Vercel Preview env', 'Railway staging env', 'EAS preview env', 'Supabase staging project', 'Sentry staging projects'],
  ...
  },
  {
    ...
    secretStores: ['Vercel Production env', 'Railway production env', 'EAS production env', 'Supabase production project', 'provider dashboards'],
    publicEnv: ['NEXT_PUBLIC_APP_URL', 'NEXT_PUBLIC_API_URL', 'NEXT_PUBLIC_SUPABASE_URL', 'NEXT_PUBLIC_SUPABASE_ANON_KEY', 'EXPO_PUBLIC_* production values'],
    serverEnv: ['DATABASE_URL production', 'DIRECT_DATABASE_URL production', 'payment secrets', 'telecom secrets', 'webhook secrets', 'S3 secrets', 'SENTRY_DSN'],
...
export const RELEASE_GATES: ReleaseGateDefinition[] = [
  ...
  },
  {
    ...
    phase: 'data',
    required: true,
    check: 'Supabase migrations, RLS policies, and storage changes are reviewed, applied to staging first, and do not require destructive rollback.',
  },
  {
    id: 'webhook-smoke-test',
    phase: 'api',
    required: true,
    check: 'Twilio, payment, and provider webhook URLs use the deployed Railway API and verify signatures where secrets exist.',
  },
  {
    id: 'auth-browser-smoke',
    phase: 'web',
  ...
  {
    id: 'revenuecat-subscriptions',
    phase: 'mobile',
    required: true,
    check: 'RevenueCat purchase, restore, webhook sync, and entitlement refresh are verified on staged or production mobile builds.',
  },
  {
...
export const OBSERVABILITY_CHECKS: ObservabilityCheckDefinition[] = [
  ...
  },
  {
    id: 'provider-health',
    surface: 'provider',
    signal: 'Twilio, Telnyx, Bandwidth, Airalo, Oxylabs, Smartproxy, and WireGuard health/degraded states are visible to operators.',
  ...
  },
  {
    ...
    surface: 'database',
    signal: 'Supabase Postgres connection pool is stable; migrations, RLS policies, and audit tables are present.',
  ...
  {
    id: 'revenuecat-webhooks',
    surface: 'payments',
    signal: 'RevenueCat webhook delivery, authorization mismatches, sync failures, and entitlement drift are observable.',
    requiredEnv: ['REVENUECAT_SECRET_API_KEY', 'REVENUECAT_PROJECT_ID'],
    alertOwner: 'mobile-billing',
    releaseBlocker: true,
  },
];
