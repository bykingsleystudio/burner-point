...
export const DEPLOYMENT_TARGETS: DeploymentTargetDefinition[] = [
  ...
  },
  {
    ...
    productionRequired: true,
    releaseCheck: 'Email, phone, Google, Apple, Microsoft, redirect URLs, and webhook signing are configured per environment.',
  ...
  },
  {
    ...
    stage: 'primary',
    environmentScope: ['staging', 'production'],
    ownerSurface: 'Global SMS and voice verification route expansion',
    requiredEnv: ['INFOBIP_BASE_URL', 'INFOBIP_API_KEY'],
    optionalEnv: ['INFOBIP_WEBHOOK_SECRET', 'INFOBIP_SENDER_ID'],
    releaseCheck: 'Country/service route test succeeds and delivery callbacks are recorded.',
    rollback: 'Remove Infobip from route selection while keeping Twilio/Vonage active.',
  },
  {
    id: 'vonage',
    name: 'Vonage',
    ...
    stage: 'fallback',
    environmentScope: ['staging', 'production'],
    ownerSurface: 'Independent SMS/voice/verify fallback',
    requiredEnv: ['VONAGE_API_KEY', 'VONAGE_API_SECRET'],
    optionalEnv: ['VONAGE_APPLICATION_ID', 'VONAGE_PRIVATE_KEY', 'VONAGE_WEBHOOK_SECRET'],
    productionRequired: true,
    releaseCheck: 'Fallback route is exercised in staging without relying on the Twilio outage domain.',
  ...
  },
  {
    ...
    stage: 'primary',
    environmentScope: ['staging', 'production'],
    ownerSurface: 'US/Canada number infrastructure and long-term number quality',
    requiredEnv: ['BANDWIDTH_ACCOUNT_ID', 'BANDWIDTH_API_TOKEN'],
    optionalEnv: ['BANDWIDTH_APPLICATION_ID', 'BANDWIDTH_WEBHOOK_SECRET'],
    releaseCheck: 'Inventory, assignment, and webhook callbacks are verified for a test number.',
    rollback: 'Keep existing Twilio inventory active while Bandwidth route is disabled.',
  },
  {
    id: 'openai',
    name: 'OpenAI',
  ...
  },
  {
    ...
    optionalEnv: ['BRIGHTDATA_BASE_URL', 'BRIGHTDATA_CUSTOMER_ID', 'BRIGHTDATA_ZONE', 'BRIGHTDATA_WEBHOOK_SECRET'],
...
export const OBSERVABILITY_CHECKS: ObservabilityCheckDefinition[] = [
  ...
  },
  {
    id: 'provider-health',
    surface: 'provider',
    signal: 'Twilio, Infobip, Vonage, Bandwidth, 1GLOBAL, Bright Data, and WireGuard health/degraded states are visible to operators.',
